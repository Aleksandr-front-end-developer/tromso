    ...
    function getHeight(showMoreBlock, showMoreContent) {
      ...
      if (showMoreType === "items") {
        ...
        for (let index = 1; index <= showMoreItems.length; index++) {
          ...
          hiddenHeight += showMoreItem.offsetHeight + parseInt(window.getComputedStyle(showMoreItem).getPropertyValue("margin-bottom"));

          if (index == Number(showMoreTypeValue)) break;
        }
      } else {
        const showMoreTypeValue = showMoreContent.dataset.showmoreContent ? showMoreContent.dataset.showmoreContent : 150;
        hiddenHeight = showMoreTypeValue;
      }
      if (window.matchMedia("(max-width: 767px)").matches && [...showMoreBlock.classList].find((value) => value == "article__block")) {
        if (hiddenHeight) return hiddenHeight + 12;
        return hiddenHeight;
      }
      if (window.matchMedia("(min-width: 768px)").matches && [...showMoreBlock.classList].find((value) => value == "article__block")) {
        if (hiddenHeight) return hiddenHeight + 32;
        return hiddenHeight;
      }
...
export function scrollSidebar() {
  const sidebar = document.querySelector(".trending.fixed");

  if (!sidebar || window.matchMedia("(max-width: 61.99875em)").matches) return;
...
function Ascroll() {
  const a = document.querySelector(".trending.fixed");
  if (!a) return;
  let Ra = a.getBoundingClientRect(),
    R1bottom = document.querySelector(".entry-content").getBoundingClientRect().bottom;
  if (Ra.bottom < R1bottom) {
    if (b == null) {
      ...
      a.style.padding = "0";
      a.style.border = "0";
      const main = document.querySelector(".js-container-trending.fixed");
